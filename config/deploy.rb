...
set :rvm_ruby_string, '1.9.3-p0@spree-edge'
set :rvm_type, :user
...
after 'deploy:rake_tasks', 'deploy:symlink_shared'
after 'deploy:symlink_shared', 'nginx:config'
...
namespace :deploy do
  ...
  task :rake_tasks do
    run "cd #{release_path} && RAILS_ENV=production bundle exec rake db:migrate db:seed"
  end
end
